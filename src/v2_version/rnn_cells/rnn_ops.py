...
def get_variable(name, shape, initializer=None, dtype=tf.float64, device=None):
    ...
    Args:
      ...
      dtype: data type, defaults to tf.float64
    ...
    if initializer is None:
        with tf.device(device):
            var = tf.compat.v1.get_variable(name, shape, dtype=dtype)
    else:
        with tf.device(device):
...
def linear(args, output_size, bias, weights_init=None, bias_start=0.0):
    ...
    with tf.compat.v1.variable_scope(scope) as outer_scope:
        ...
        with tf.compat.v1.variable_scope(outer_scope) as inner_scope:
            ...
            biases = get_variable('Biases', [output_size], initializer=tf.compat.v1.constant_initializer(bias_start, dtype=dtype))
        return tf.nn.bias_add(res, biases)


def create_initial_state(batch_size, state_size, trainable=True, initializer=tf.compat.v1.random_normal_initializer()):
...
def layer_norm(x, axes=1, initial_bias_value=0.0, epsilon=1e-3, name="var"):
    ...
    with tf.compat.v1.variable_scope(scope):
        with tf.compat.v1.variable_scope(name):
            ...
            variance = tf.sqrt(tf.reduce_mean(input_tensor=tf.square(x - mean), axis=axes, keepdims=True))

            with tf.device('/cpu:0'):
                gain = tf.compat.v1.get_variable('gain', x.get_shape().as_list()[1:],
                                       initializer=tf.compat.v1.constant_initializer(1.0))
                bias = tf.compat.v1.get_variable('bias', x.get_shape().as_list()[1:],
                                       initializer=tf.compat.v1.constant_initializer(initial_bias_value))
