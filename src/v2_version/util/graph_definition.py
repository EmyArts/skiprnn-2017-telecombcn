...
def create_generic_flags():
    ...
    tf.app.flags.DEFINE_float('suprisal_cost', 0.0001, "How much it gets punished for average surprisal or discarded samples.")
    tf.app.flags.DEFINE_float('use_surprisal', False, "Whether to use surprisal punishment on loss function")
...
def compute_gradients(loss, learning_rate, gradient_clipping=-1):
    ...
    if gradient_clipping > 0:
        vars_to_optimize = tf.compat.v1.trainable_variables()
        grads, _ = tf.clip_by_global_norm(tf.gradients(ys=loss, xs=vars_to_optimize), clip_norm=gradient_clipping)
...
def create_model(model, num_cells, batch_size, learn_initial_state=True):
    ...
    if isinstance(num_cells, list) and len(num_cells) > 1:
        if model == 'skip_lstm':
            ...
            cells = tf.compat.v1.nn.rnn_cell.MultiRNNCell(cell_list)
        elif model == 'gru':
...
def compute_budget_loss(model, loss, updated_states, cost_per_sample):
    ...
    """
    if using_skip_rnn(model):
        return tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=cost_per_sample * updated_states, axis=1), axis=0)
    else:
